...
import { libraryRouteForId, parseLibraryRouteId } from '@/features/library/lib/library-route'
...
const SIDEBAR_LIBRARY_ORDER_KEY = 'bookorbit:sidebar:libraries:order'
...
const { libraries, fetchLibraries, refreshLibraries, reorderLibraries } = useLibraries({ includeSourceBacked: true })
...
function readStoredLibraryOrder(): number[] {
  try {
    const parsed = JSON.parse(localStorage.getItem(SIDEBAR_LIBRARY_ORDER_KEY) ?? '[]')
...
function sortLibrariesByStoredOrder(items: Library[]): Library[] {
  const storedOrder = readStoredLibraryOrder()
...
const sidebarLibraries = computed(() => sortLibrariesByStoredOrder(libraries.value))

function reorderFilesystemLibraries(order: { id: number; displayOrder: number }[]): Promise<void> {
  localStorage.setItem(SIDEBAR_LIBRARY_ORDER_KEY, JSON.stringify(order.map(({ id }) => id)))
  return reorderLibraries(order.filter(({ id }) => id > 0))
}
...
} = useDraggableOrder({ source: sidebarLibraries, persist: reorderFilesystemLibraries })
...
const isRequestsActive = computed(() => route.name === 'requests')
const isToolsActive = computed(() => typeof route.name === 'string' && route.name.startsWith('tools-'))

const activeLibraryId = computed(() => {
  const id = route.params.id
  return (route.name === 'library' || route.name === 'library-item-detail' || route.name === 'library-reader') && id ? parseLibraryRouteId(id) : null
})
      ...
      <SidebarGroup>
        <SidebarGroupContent>
          <SidebarMenu class="gap-0">
            ...
            />
            <SidebarNavItem
              :is-active="isRequestsActive"
              tooltip="Requests"
              :icon="Icons.Inbox"
              label="Requests"
              @click="navigateFromSidebar({ name: 'requests' })"
            />
            <SidebarNavItem
            ...
            <SidebarGroupContent>
              ...
              >
                <SidebarNavItem
                  ...
                  :icon-class="''"
                  :label="lib.name"
                  @click="navigateFromSidebar(libraryRouteForId(lib.id))"
                >
                  <template #badge>
                    <span
