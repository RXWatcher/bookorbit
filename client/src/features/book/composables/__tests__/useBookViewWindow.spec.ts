...
import { CLOUD_COMIC_LIBRARY_ID, type BookCard, type BooksPage, type GroupRule, type JumpBucket } from '@bookorbit/types'
...
describe('useBookViewWindow', () => {
  ...
  it('loads source-backed library ids through the normal book window', async () => {
    mockApi(75)
    setup({ scope: CLOUD_COMIC_LIBRARY_ID })
    await flush()

    const [url] = fetchMock.mock.calls[0] ?? []
    expect(url).toBe(`/api/v1/libraries/${CLOUD_COMIC_LIBRARY_ID}/books`)
  })
