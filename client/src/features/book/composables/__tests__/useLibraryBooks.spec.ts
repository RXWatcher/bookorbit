...
describe('useLibraryBooks', () => {
  ...
  it('allows the first non-reset load before total is known', async () => {
    const { useLibraryBooks } = await import('../useLibraryBooks')
    const libraryId = ref<number | null>(1)
    const { hasMore, initialized, items, load } = useLibraryBooks(libraryId)

    expect(initialized.value).toBe(false)
    expect(hasMore.value).toBe(true)
    mockOkResponse(makePage([makeBook({ id: 1, title: 'First page' })], 1))

    await load()

    expect(fetchMock).toHaveBeenCalledTimes(1)
    expect(items.value).toHaveLength(1)
    expect(items.value[0]!.title).toBe('First page')
