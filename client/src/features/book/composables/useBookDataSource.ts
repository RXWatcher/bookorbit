...
import type { BookCard, BookQuery, SortSpec } from '@bookorbit/types'
...
export function useBookDataSource<TItem = BookCard>(
  scopeId: Ref<number>,
  endpointForId: (id: number) => string,
  options: BookDataSourceOptions = {},
) {
  ...
  async function load(reset = false) {
    ...
    try {
      ...
      const data: BookDataPage<TItem> = await res.json()
      if (controller.signal.aborted) return
