...
import type { BookCard, BookQuery, GroupRule, LibraryBookItem, LibraryBooksPage, SortField, SortSpec } from '@bookorbit/types'
...
export function useLibraryBooks<TItem extends LibraryBookItem = BookCard>(
  ...
  const hasMore = computed(() => libraryId.value !== null && (!initialized.value || items.value.length < total.value))

  let activeController: AbortController | null = null

  async function load(reset = false) {
    if (libraryId.value === null) return
    if (!reset && (loading.value || (initialized.value && !hasMore.value))) return
    ...
    try {
      ...
      const page: LibraryBooksPage = await res.json()

      if (controller.signal.aborted) return

      if (pagination.page === 0) {
        items.value = page.items as TItem[]
      } else {
